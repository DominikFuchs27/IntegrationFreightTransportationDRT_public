...
def CalculateRegretValue(route_list_input,removed_nodes, demand_pass,demand_pack, time_window, mydict):
    ...
    for i in range(len(route_list_input)):
        ...
        route_testing, best_index_node, delta_cost = ALNS_Sub_Parallel.GreedyInsertion(route_testing)   # Insert the removed nodes in the current route
    ...
    index_best_route = min(enumerate(delta_cost_list), key=itemgetter(1))[0]   # index of the least increase in costs
    index_insertion_nodes = best_index_node_list[index_best_route]
    index_second_best_route = int(heapq.nsmallest(4,enumerate(delta_cost_list),key=itemgetter(1))[1][0])    # index of second best route
    regret_value = delta_cost_list[index_second_best_route] - delta_cost_list[index_best_route]     # calculate regret value
...
def RegretHeuristic(route_list_input,demand_pass, demand_pack, time_window, mydict, unfitting_nodes, removed_nodelist):
    ...
    while removed_nodelist:                     # while there are still nodes in the list removed_nodelist
        regret_value_list = []
        ...
        for k in range(int(len(removed_nodelist)/2)):
            ...
            if denied_nodes_outcome:    # if there are any denied nodes --> store as denied nodes
                denied_nodes.append(denied_nodes_outcome)
        ...
        index_regret_maximum = max(enumerate(regret_value_list), key=itemgetter(1))[0]      # find the value with the highest regret value
        removed_nodes = [removed_nodelist[index_regret_maximum*2],removed_nodelist[index_regret_maximum*2+1]]
        index_overall_best_route = index_best_route_list[index_regret_maximum]      # find route with the highest regret value
        ...
        if len(index_best_insertion_nodes_list[index_regret_maximum]) < 2:
            ...
            new_route = Route.route(new_starting_nodes, demand_pass_route, mydict, time_window_route, demand_pack_route) # create new route
            ...
            new_route = HelpingFunctions.UpdateRoute(new_route)  # update the values of the route
        ...
        for i in range(len(route_list_input)):              # Update Route if it was not previosuly changed --> need to adapt time_earlist_dep, because nodes have been removed
            if len(route_list_input[i].current_nodes) == 0:   # if no nodes in the route --> set all variables to 0
